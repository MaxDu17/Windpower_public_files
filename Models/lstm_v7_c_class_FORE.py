...
class LSTM:
    ...
    def graph(self, hyperparameters, sess):
        ...
        with tf.name_scope("weights_and_biases"):
            W_Forget_and_Input = tf.Variable(tf.random_normal(shape = [hidden_dim + cell_dim + 21,cell_dim]), name = "forget_and_input_weight") #note that forget_and_input actually works for forget, and the input is the inverse
            W_Output = tf.Variable(tf.random_normal(shape=[hidden_dim + cell_dim + 21,cell_dim]), name="output_weight")
            W_Gate = tf.Variable(tf.random_normal(shape=[hidden_dim + cell_dim + 21,cell_dim]), name="gate_weight")
