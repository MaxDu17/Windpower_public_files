...
NAME = "lstm_v9_c_class" #this is the name of the python file for logging purposes

k = open("../Genetic/lstm_v9_c_classbest.csv", "r")
...
with tf.name_scope("weights_and_biases"):
    W_Forget_and_Input = tf.Variable(tf.random_normal(shape=[hidden_dim + 1, cell_dim]),
    ...
    W_Carry = tf.Variable(tf.random_normal(shape=[hidden_dim + 1, cell_dim]), name="carry_weight")
...
def step(last_state, X):  # this is the function for each node of the LSTM
    ...
    with tf.name_scope("output_gate"):  # output gate values to hidden
        ...
        current_hidden = tf.add(Carry_over, cell_propagated_hidden,
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        if epoch % 50 == 0 and epoch > epochs-(50*hyp.FINALJUMP): #this finds entropic local minima at the end and saves them
            ...
            saver.save(sess, "../Graphs_and_Results/" + NAME + "/models/V9Genetic", global_step=epoch)
